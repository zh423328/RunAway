...
public class Item : MonoBehaviour {
	...
	public void ItemGet(){
		if(GameAttribute.gameAttribute.deleyDetect == false){
			if(typeItem == TypeItem.Coin){
                ...
                if (ControllerPlayer.instace.isRoll == false)
                {
					HitObstacle();
					//Play sfx when get hit
					SoundManager.instance.PlayingSound("HitOBJ");
	...
	private void HitCoin(){
        if (ControllerPlayer.instace.isMultiply == false)
        {
	...
	private void initEffect(GameObject prefab){
        GameObject go = (GameObject)Instantiate(prefab, ControllerPlayer.instace.transform.position, Quaternion.identity);
        go.transform.parent = ControllerPlayer.instace.transform;
