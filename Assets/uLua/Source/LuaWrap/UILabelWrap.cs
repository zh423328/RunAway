...
public class UILabelWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("finalFontSize", get_finalFontSize, null),
			...
			new LuaField("overflowEllipsis", get_overflowEllipsis, set_overflowEllipsis),
	...
	static int get_finalFontSize(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name finalFontSize");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index finalFontSize on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.finalFontSize);
		return 1;
	}
	...
	static int get_overflowEllipsis(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name overflowEllipsis");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index overflowEllipsis on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.overflowEllipsis);
		return 1;
	}
	...
	static int set_overflowEllipsis(IntPtr L)
	{
		...
		obj.overflowEllipsis = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
